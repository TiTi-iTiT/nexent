...
class EmbeddingAdapter(MultimodalAdapter, HttpTransportMixin):
    ...
    def _make_request(self, data: Dict[str, Any], timeout: Optional[float] = None) -> Dict[str, Any]:
        ...
        """
        # Keep provider-specific JSON fields, but protect the protocol fields
        # generated by the adapter from accidental custom-parameter overrides.
        request_data = {**(self._context.extra_body or {}), **data}
        response = self._session.post(
            self._base_url, headers=self._headers, json=request_data, timeout=timeout, verify=self._ssl_verify
        )
